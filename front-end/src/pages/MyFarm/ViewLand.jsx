  ...
  return (
    <div style={premiumStyles.pageWrapper} onClick={() => setShowMenuId(null)}>
      <style>{`
        ...
        .land-menu-item { padding: 12px 18px; display: flex; align-items: center; gap: 12px; font-size: 14px; cursor: pointer; color: #333; }
      `}</style>
        ...
        <div className="land-grid">
          {filteredLands.map((plot) => (
            <div key={plot.id} className="alibaba-card">
              ...
              {showMenuId === plot.id && (
                <div className="land-dropdown-menu" ref={menuRef}>
                  <div className="land-menu-item" onClick={() => setEditingPlotId(plot.id)}>
                    <FaEdit color="#007185"/> Edit Asset
                  </div>
                  <div className="land-menu-item" style={{color: '#d32f2f', fontWeight: 'bold'}} onClick={() => handleDrop(plot.id)}>
                    <FaTrash /> DROP Node
                  </div>
                </div>
              )}
