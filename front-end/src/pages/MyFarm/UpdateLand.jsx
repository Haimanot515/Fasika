...
const UpdateLand = ({ plotId, onUpdateSuccess, onCancel }) => {
    ...
    if (loading) return (
        <div style={styles.fullscreenOverlay}>
            <Loader2 style={{ animation: 'spin 1s linear infinite', color: '#fff' }} size={48} />
        </div>
    );
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div style={styles.buttonActionArea}>
                                <button type="submit" disabled={submitting} style={styles.submitBtn}>
                                    {submitting ? (
                                        <><Loader2 style={{ animation: 'spin 1s linear infinite' }} size={24}/> SYNCING REGISTRY...</>
                                    ) : (
...
const styles = {
    ...
    closeBtn: { background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', padding: '5px' },
    scrollBody: {
        flex: 1,
        overflowY: 'auto',
        padding: '60px 20px',
    ...
    sectionHeader: { fontSize: '15px', fontWeight: '800', color: '#166534', textTransform: 'uppercase', letterSpacing: '1.2px', marginBottom: '25px', display: 'flex', alignItems: 'center', gap: '12px' },
    grid2: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '25px', marginBottom: '25px' },
    grid4: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '35px' },
    inputGroup: { display: 'flex', flexDirection: 'column', gap: '10px' },
    label: { fontSize: '13px', fontWeight: '700', color: '#64748b' },
    input: { width: '100%', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#f8fafc', outline: 'none', fontSize: '15px' },
    assetRow: { display: 'flex', gap: '15px', marginBottom: '15px', alignItems: 'center' },
    ...
    hr: { margin: '45px 0', border: '0', borderTop: '1px solid #e2e8f0' },
    fileUploadBox: { padding: '40px', border: '3px dashed #e2e8f0', borderRadius: '20px', textAlign: 'center', background: '#fcfcfc' },
    buttonActionArea: { display: 'flex', gap: '20px', marginTop: '50px' },
    submitBtn: { flex: 2, padding: '20px', backgroundColor: '#166534', color: 'white', border: 'none', borderRadius: '16px', cursor: 'pointer', fontWeight: '800', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px' },
    cancelBtn: { flex: 1, padding: '20px', backgroundColor: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: '16px', cursor: 'pointer', fontWeight: '800', fontSize: '16px' }
};
