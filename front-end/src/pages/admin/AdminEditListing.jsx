...
const AdminEditListing = () => {
  ...
  const theme = {
    ...
    glassCard: { width: "100%", maxWidth: "850px", background: "#ffffff", borderRadius: "16px", boxShadow: "0 10px 25px rgba(0,0,0,0.05)", border: "1px solid #e2e8f0", overflow: "hidden" },
    ...
    <div style={theme.container}>
      <div style={theme.glassCard}>
        <div style={theme.header}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: "22px" }}>EDIT REGISTRY NODE</h1>
            <p style={{ margin: 0, opacity: 0.8, fontSize: "12px" }}>AUTHORITY ID: {listing_id}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpdate} style={{ padding: "40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <div style={theme.uploadBox} onClick={() => document.getElementById("p_img_admin").click()}>
              {primaryPreview ? (
                <img src={primaryPreview} alt="Preview" style={{ width: "100%", height: "100px", objectFit: "cover", borderRadius: "8px" }} />
              ) : (
                <><HiOutlinePhotograph size={24} color="#3b82f6" /><span>Replace Cover</span></>
              )}
              <input id="p_img_admin" type="file" hidden accept="image/*" onChange={(e) => {
            ...
            <div style={{ ...theme.uploadBox, background: "#f8fafc", borderColor: "#cbd5e1" }}>
               ...
               <span style={{fontSize: "12px"}}>Inject to Gallery ({galleryImages.length})</span>
